...
public class UpdateProductStockUseCaseTests
{
    ...
    private readonly UpdateProductStockUseCase _sut;

    public UpdateProductStockUseCaseTests()
    {
        _sut = new UpdateProductStockUseCase(_repoMock.Object);
    }

    [Fact]
    public async Task ExecuteAsync_WithExistingProduct_UpdatesStock()
    {
        var productId = Guid.NewGuid();
        var product = new ProductEntity("Produto A", null, 50m, 10);
        var input = new UpdateProductStockInput { Stock = 25 };
    ...
    public async Task ExecuteAsync_WithNonExistingId_ThrowsNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() =>
            _sut.ExecuteAsync(productId, new UpdateProductStockInput { Stock = 10 }));
    }
}
